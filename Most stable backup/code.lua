...
--This stops the annoying bug when in Order Hall and can't press escape to log out...
...
-- 		end
    ...
    -- Main Chunk of code for addon functions. 
  	...
  	if event == "ACTIVE_TALENT_GROUP_CHANGED" or
		...
		elseif class == "WARLOCK" then
			EditMacro("WSxSwapper",nil,nil,"#show Soul Leech\n/run local a={\"Rebellious Imp\",\"Lesser Voidcaller\",\"Netherspace Abyssal\",\"Horde Fanatic\",\"Eye of the Legion\",\"Sister of Temptation\",\"Nibbles\"}b=math.random(1,#a)_,c=C_PetJournal.FindPetIDByName(a[b])do C_PetJournal.SummonPetByGUID(c)end")
		elseif class == "MONK" then
		...
		if class == "DEATHKNIGHT" then
			if playerspec == 1 then
				EditMacro("WSxSwapper",nil,nil,"#showtooltip On a Pale Horse\n/run local a={\"Bloodbrood Whelpling\",\"Lost of Lordaeron\",\"Mr. Bigglesworth\",\"Boneshard\",\"Grotesque\",\"Stinkrot\",\"Unborn Val'kyr\"}b=math.random(1,#a)_,c=C_PetJournal.FindPetIDByName(a[b])do C_PetJournal.SummonPetByGUID(c)end")
			elseif playerspec == 2 then
				EditMacro("WSxSwapper",nil,nil,"#showtooltip On a Pale Horse\n/run local a={\"Frostbrood Whelpling\",\"Lost of Lordaeron\",\"Mr. Bigglesworth\",\"Boneshard\",\"Landro's Lichling\",\"Stinkrot\",\"Unborn Val'kyr\"}b=math.random(1,#a)_,c=C_PetJournal.FindPetIDByName(a[b])do C_PetJournal.SummonPetByGUID(c)end")
			else
				EditMacro("WSxSwapper",nil,nil,"#showtooltip On a Pale Horse\n/run local a={\"Vilebrood Whelpling\",\"Lost of Lordaeron\",\"Mr. Bigglesworth\",\"Boneshard\",\"Grotesque\",\"Stinkrot\",\"Unborn Val'kyr\"}b=math.random(1,#a)_,c=C_PetJournal.FindPetIDByName(a[b])do C_PetJournal.SummonPetByGUID(c)end")
			end
		end
		...
		if class == "DEMONHUNTER" then
			EditMacro("WSxSwapper",nil,nil,"#show [spec:1]Darkness;Sigil of Misery\n/run local a={\"Murkidan\",\"Emmigosa\",\"Abyssius\",\"Micronax\",\"Wyrmy Tunkins\",\"Fragment of Desire\",\"Hateful Eye\"}b=math.random(1,#a)_,c=C_PetJournal.FindPetIDByName(a[b])do C_PetJournal.SummonPetByGUID(c)end")
		end
		...
		if class == "PALADIN" then
			if playerspec == 1 then
				EditMacro("WSpecs!",nil,nil,"/settitle [spec:1]The Light of Dawn\n/equipset [spec:1]Noon!\n/run x=1 if(IsShiftKeyDown())then x=2 elseif(IsControlKeyDown())then x=3 end if(x~=GetSpecialization())then SetSpecialization(x) end")
			elseif playerspec == 2 then
				EditMacro("WSpecs!",nil,nil,"/settitle [spec:2]Lord of The Reins\n/equipset [spec:2]DoubleGate\n/run x=1 if(IsShiftKeyDown())then x=2 elseif(IsControlKeyDown())then x=3 end if(x~=GetSpecialization())then SetSpecialization(x) end")
			else
		...
		if class == "HUNTER" then
			...
			elseif playerspec == 2 then
				EditMacro("WSpecs!",nil,nil,"/settitle [spec:2]Terror of the Tushui\n/equipset [spec:2]DoubleGate\n/run x=1 if(IsShiftKeyDown())then x=2 elseif(IsControlKeyDown())then x=3 end if(x~=GetSpecialization())then SetSpecialization(x) end")
			else
		...
		if class == "SHAMAN" then
			EditMacro("WArtifactCDs",nil,nil,"#showtooltip\n/targetenemy [noexists]\n/use [spec:1] Stormkeeper; [spec:2] Doom Winds; [spec:3,@cursor] Gift of the Queen;")
		elseif class == "MAGE" then
			EditMacro("WArtifactCDs",nil,nil,"#showtooltip\n/targetenemy [noexists]\n/use [spec:1] Mark of Aluneth; [spec:2] Phoenix's Flames; [spec:3] Ebonbolt;\n/equip [spec:1] Aluneth")
		elseif class == "WARLOCK" then
			EditMacro("WArtifactCDs",nil,nil,"#showtooltip\n/targetenemy [noexists]\n/use [spec:1] Reap Souls; [spec:2] Thal'kiel's Consumption; [spec:3] Dimensional Rift;")
		elseif class == "MONK" then
			EditMacro("WArtifactCDs",nil,nil,"#showtooltip\n/targetenemy [noexists]\n/use [spec:1,@cursor] Exploding Keg; [@mouseover,help,nodead,spec:2][spec:2] Sheilun's Gift; [spec:3] Strike of the Windlord;")
		elseif class == "PALADIN" then
			EditMacro("WArtifactCDs",nil,nil,"#showtooltip\n/targetenemy [noexists]\n/use [spec:1,@mouseover,help,nodead][spec:1] Tyr's Deliverance; [spec:2] Eye of Tyr; [spec:3] Wake of Ashes;")
		elseif class == "HUNTER" then
			EditMacro("WArtifactCDs",nil,nil,"#showtooltip\n/targetenemy [noexists]\n/use [spec:1] Titan's Thunder; [spec:2] Windburst; [spec:3] Fury of the Eagle;\n/use [nocombat,noexists]Ruthers' Harness")
		elseif class == "ROGUE" then
			EditMacro("WArtifactCDs",nil,nil,"#showtooltip\n/targetenemy [noexists]\n/use [spec:1] Kingsbane; [spec:2] Curse of the Dreadblades; [spec:3] Goremaw's Bite;")
		elseif class == "PRIEST" then
			EditMacro("WArtifactCDs",nil,nil,"#showtooltip\n/targetenemy [noexists]\n/use [spec:1] Light's Wrath(Artifact); [spec:2,@mouseover,help,nodead][spec:2] Light of T'uure; [spec:3] !Void Torrent;")
		elseif class == "DEATHKNIGHT" then
			EditMacro("WArtifactCDs",nil,nil,"#showtooltip\n/targetenemy [noexists]\n/use [spec:1] Consumption; [spec:2] Sindragosa's Fury; [spec:3] Apocalypse(Artifact);")
		elseif class == "WARRIOR" then
			EditMacro("WArtifactCDs",nil,nil,"#showtooltip\n/targetenemy [noexists]\n/use [spec:1] Warbreaker; [spec:2] Odyn's Fury; [spec:3] Neltharion's Fury;")
		elseif class == "DRUID" then
			EditMacro("WArtifactCDs",nil,nil,"#showtooltip\n/targetenemy [noexists]\n/use [spec:1] New Moon; [spec:2] Ashamane's Frenzy; [spec:3] Rage of the Sleeper; [spec:4] Essence of G'Hanir;")
		elseif class == "DEMONHUNTER" then
			EditMacro("WArtifactCDs",nil,nil,"#showtooltip\n/targetenemy [noexists]\n/use [spec:1] Fury of the Illidari; [spec:2] Soul Carver;")
		end
		...
		-- Ctrl+Q, Skillbomb for all classes, Completed
		if class == "SHAMAN" then
			EditMacro("WSkillbomb",nil,nil,"#showtooltip\n/use [spec:1] Fire Elemental\n/use Ascendance\n/use Elemental Mastery\n/use Blood fury\n/use Ai-Li's Skymirror\n/use Rukhmar's Sacred Memory\n/use 13\n/use Flippable Table\n/use Big Red Raygun\n/use Berserking")
		elseif class == "MAGE" then
		...
		elseif class == "ROGUE" then
			EditMacro("WSkillbomb",nil,nil,"#showtooltip\n/use [spec:2]Adrenaline Rush;[spec:3]Shadow Blades;[spec:1]Vendetta;\n/use [nostealth] Will of Northrend\n/use [nostealth] Berserking\n/use [nostealth] Ai-Li's Skymirror\n/use [nostealth] Rukhmar's Sacred Memory\n/use [nostealth] 13")
		elseif class == "PRIEST" then
			EditMacro("WSkillbomb",nil,nil,"#showtooltip\n/use Shadowfiend\n/use Power Infusion\n/use Berserking\n/use Ai-Li's Skymirror\n/use Rukhmar's Sacred Memory\n/use 13\n/use Big Red Raygun")
		elseif class == "DEATHKNIGHT" then
			EditMacro("WSkillbomb",nil,nil,"#showtooltip\n/use [spec:1]Dancing Rune Weapon;[spec:2]Pillar of Frost;\n/castsequence [spec:3] Dark Transformation, Summon Gargoyle\n/use Ai-Li's Skymirror\n/use 13\n/use Combat Ally\n/use Will of Northrend")
		elseif class == "WARRIOR" then
			EditMacro("WSkillbomb",nil,nil,"#showtooltip Battle Cry\n/use Bloodbath\n/use Dragon Roar\n/use Avatar\n/use Will of Northrend\n/use Ai-Li's Skymirror\n/use 13\n/use [nospec:2,talent:7/3,@player]Ravager;Bladestorm\n/use Flippable Table")
		elseif class == "DRUID" then
			EditMacro("WSkillbomb",nil,nil,"#showtooltip\n/use [spec:1]Celestial Alignment;[spec:2/3]Will of Northrend;[spec:4]Treant Form;\n/use Berserking\n/use Ai-Li's Skymirror\n/use Rukhmar's Sacred Memory\n/use 13\n/use [spec:2]Berserk\n/use Big Red Raygun\n/use [@cursor]Force of Nature")
		elseif class == "DEMONHUNTER" then
		...
		elseif class == "DRUID" then
			EditMacro("WSxT30",nil,nil,"#showtooltip [talent:2/3]Wild Charge;[talent:2/2]Displacer Beast;Renewal\n/use Orb of the Sin'dorei\n/use [nocombat]Mylune's Call\n/use [nocombat,noexists]Cooking School Bell")
		elseif class == "DEMONHUNTER" then
		...
		elseif class == "HUNTER" then
			EditMacro("WSxT45",nil,nil,"#showtooltip [mod:ctrl]Goblin Fishing Bomb;[mod:shift,nospec:3]Misdirection;Stopping Power\n/use [help,nodead][@focus,help,nodead][pet,@pet]Misdirection;\n/use Goblin Fishing Bomb\n/use Angler's Fishing Spear")
		elseif class == "ROGUE" then
			EditMacro("WSxT45",nil,nil,"#showtooltip\n/use [stance:0,combat,spec:3]Shadow Dance;[nospec:3,nostance:1/2/3,combat]Vanish;[stance:0,nocombat]Stealth;[mod:alt,@focus,harm,nodead,stance:1/2/3][spec:1,stance:1/2/3]Garrote;[stance:1/2/3][]Cheap Shot;")
		end
		...
		elseif class == "DEMONHUNTER" then
			EditMacro("WSxT60",nil,nil,"#showtooltip [nocombat,noexists]Spire of Spite;[spec:2,talent:4/2]Fracture\n/use Spire of Spite\n/script PetDismiss();\n/cry")
		end
		...
		elseif class == "HUNTER" then
			EditMacro("WSxT75",nil,nil,"/use [pet:Core Hound]Ancient Hysteria;[pet:Nether Ray]Netherwinds;[pet]Drums of the Mountain;[nopet]Call Pet 5;\n/use [nopet:Core Hound,combat][nopet:Nether Ray,combat] Drums of Fury\n/use [nopet:Core Hound,combat][nopet:Nether Ray,combat] Drums of the Mountain;")
		elseif class == "ROGUE" then
		...
		elseif class == "PALADIN" then
			EditMacro("Wz10PvP2",nil,nil,"#showtooltip\n/use [spec:2,pvptalent:3/1]Shield of Virtue;\n/stopmacro [combat]\n/kneel\n/use Sylvanas' Music Box")
		elseif class == "HUNTER" then
		...
		elseif class == "MONK" then
			EditMacro("Wz11PvP3",nil,nil,"#showtooltip\n/use [spec:1,pvptalent:5/1]Guard;[spec:1,pvptalent:5/3]Craft: Nimble Brew;")
		elseif class == "PALADIN" then
		...
		elseif class == "PRIEST" then
			EditMacro("Wz11PvP3",nil,nil,"#showtooltip\n/use [nocombat,noexists]Thistleleaf Branch;[spec:1,pvptalent:4/2]Premonition;[spec:2,pvptalent:4/3]Greater Fade;\n/use Thistleleaf Branch")
		elseif class == "DEATHKNIGHT" then
		...
		elseif class == "DEMONHUNTER" then
			EditMacro("Wz12PvP4",nil,nil,"#showtooltip\n/use [nocombat,noexists]Micro-Artillery Controller;[spec:2,pvptalent:6/2]Demonic Trample;[spec:2,pvptalent:6/3]Illidan's Grasp;[pvptalent:6/2]Rain from Above;[pvptalent:6/3]Mana Break;")
		end
		
		-- Macro for button 1 "1"
		if class == "SHAMAN" then
			EditMacro("WSxGen1",nil,nil,"/use [spec:3,talent:1/2]Unleash Life;[noexists,nocombat]Arena Master's War Horn;[spec:2,talent:1/1]Windsong;[@mouseover,harm,nodead,nospec:2][nospec:2]Flame Shock;Lightning Bolt;\n/targetenemy [noexists]")
		elseif class == "MAGE" then
			EditMacro("WSxGen1",nil,nil,"/targetenemy [noharm]\n/use [noexists,nocombat]Set of Matches;[spec:3,talent:1/1]Ray of Frost;[spec:2]Combustion;[spec:1,talent:1/2]Presence of Mind;[spec:1,talent:1/1]Arcane Familiar;Ice Lance;\n/use 13")
		elseif class == "WARLOCK" then
		...
		elseif class == "ROGUE" then
			EditMacro("WSxGen1",nil,nil,"#showtooltip\n/use [stance:0,nocombat]Stealth;[spec:2,talent:6/1,@player]Cannonball Barrage;[spec:3]Symbols of Death;[spec:2]Pistol Shot;[noexists,nocombat]Wound Poison;[spec:1]Garrote;\n/targetenemy [noexists]\n/startattack [combat]")
		elseif class == "PRIEST" then
			EditMacro("WSxGen1",nil,nil,"#showtooltip\n/use [help,nodead]The Heartbreaker;[spec:1,talent:4/1]Power Word: Solace;[spec:2]Holy Fire;[spec:3,talent:1/3]Shadow Word: Void;The Heartbreaker;")
		elseif class == "DEATHKNIGHT" then
		...
		elseif class == "PRIEST" then
			EditMacro("WSxGen2",nil,nil,"#showtooltip [spec:3]Vampiric Touch;[spec:2]Smite;[spec:1]Power Word: Barrier;\n/cancelaura Fling Rings\n/use [nocombat] Darkmoon Ring-Flinger\n/use Smite\n/targetenemy [noexists]\n/cleartarget [dead]")
		elseif class == "DEATHKNIGHT" then
		...
		elseif class == "DRUID" then
			EditMacro("WSxGen2",nil,nil,"/use [form:1]Mangle;[form:2]Shred;[spec:2/3,talent:3/1,noform:1/2/4]Moonkin Form;[spec:2/3,talent:3/1,form:4][spec:1,@mouseover,harm,nodead][spec:1][spec:4,talent:3/1,harm,nodead]Sunfire(Solar);Healing Touch;\n/targetenemy [noexists]\n/cleartarget [dead]")
		elseif class == "DEMONHUNTER" then
		...
		elseif class == "HUNTER" then
			EditMacro("WSxSGen+2",nil,nil,"/use [spec:3,pvptalent:5/2]Mending Bandage;[pet,nopet:Spirit Beast]Dismiss Pet;[nopet]Call Pet 2;[@mouseover,help,nodead,pet][pet,help,nodead][pet,@player]Spirit Mend;\n/cancelaura x-ray specs\n/cancelaura Safari Hat\n/use Totem of Spirits")
		elseif class == "ROGUE" then
		...
		elseif class == "MAGE" then
			EditMacro("WSxGen3",nil,nil,"#showtooltip\n/use [spec:3,@cursor]Frozen Orb;[@mouseover,harm,nodead,spec:2][spec:2]Pyroblast;[spec:1,talent:7/3]Arcane Orb;Evocation;\n/targetenemy [noexists]\n/cleartarget [dead]")
		elseif class == "WARLOCK" then
		...
		elseif class == "PALADIN" then
			EditMacro("WSxGen3",nil,nil,"#showtooltip\n/use [spec:2,help,nodead,nocombat]Dalaran Disc;[spec:1,@mouseover,help,nodead][spec:1]Light of the Martyr;[spec:2]Avenger's Shield;Blade of Justice;\n/targetenemy [noexists]\n/stopspelltarget\n/cleartarget [dead]")
		elseif class == "HUNTER" then
		...
		elseif class == "ROGUE" then
			EditMacro("WSxGen3",nil,nil,"/use [stance:0,nocombat]Stealth;[stance:0,combat,spec:3]Shadow Dance;[nostance:0,spec:3]Shadowstrike;[spec:2,talent:6/1]Cannonball Barrage;[spec:2,talent:6/3]Killing Spree;[spec:2]Saber Slash;[talent:6/3]Exsanguinate;[talent:6/1]Toxic Blade;Mutilate;")
		elseif class == "PRIEST" then
		...
		elseif class == "MAGE" then
			EditMacro("WSxSGen+3",nil,nil,"/use [nocombat,noexists]Archmage Vargoth's Spare Staff;[spec:3,talent:6/1]Frost Bomb;[spec:1,talent:6/1]Nether Tempest;[spec:2,talent:6/1]Living Bomb;\n/targetenemy [noexists]")
		end
		-- Warlock Shift+3
		if class == "WARLOCK" then
			if playerspec == 1 then
		...
		elseif class == "DEATHKNIGHT" then
			EditMacro("WSxSGen+3",nil,nil,"#showtooltip\n/use [spec:2,talent:2/3]Horn of Winter;[spec:3,@mouseover,harm,nodead][spec:3]Outbreak;[spec:1,@mouseover,harm,nodead][spec:1]Death's Caress;\n/targetenemy [noexists]\n/startattack\n/stopspelltarget")
		elseif class == "WARRIOR" then
			EditMacro("WSxSGen+3",nil,nil,"/use [talent:3/3]Avatar;[spec:1,talent:3/2]Rend;[spec:3]Demoralizing Shout;\n/startattack")
		elseif class == "DRUID" then
			EditMacro("WSxSGen+3",nil,nil,"/use [form:2]Rake;[spec:4,@focus,help,nodead][spec:4]Lifebloom;\n/use [nocombat,noexists,noform:2]Cat Form; Fandral's Seed Pouch;") 
		elseif class == "DEMONHUNTER" then
		...
		elseif class == "WARLOCK" then
			EditMacro("WSxCSGen+3",nil,nil,"/use [spec:1,@focus,harm,nodead]Corruption;[spec:3,@focus,harm,nodead]Immolate;[spec:2,@focus,harm,nodead]Doom;\n/targetenemy [noharm]\n/cleartarget [dead]")
		elseif class == "MONK" then
		...
		elseif class == "WARRIOR" then
			EditMacro("WSxCSGen+3",nil,nil,"/use [@focus,harm,nodead]Rend")
		elseif class == "DRUID" then
			EditMacro("WSxCSGen+3",nil,nil,"/use [@party2,exists,nodead,spec:4]Nature's Cure;[@party2,exists,nodead]Remove Corruption")
		elseif class == "DEMONHUNTER" then
			EditMacro("WSxCSGen+3",nil,nil,"/use [@focus,harm,nodead]Throw Glaive")
		end
		...
		elseif class == "PALADIN" then
			EditMacro("WSxGen4",nil,nil,"#showtooltip\n/use [nocombat,noexists] Contemplation; Judgment;\n/targetenemy [noexists]\n/startattack\n/cleartarget [dead]")
		end
		...
		elseif class == "DRUID" then
			EditMacro("WSxGen4",nil,nil,"/use [form:1/2]Thrash;[spec:4,@mouseover,help,nodead]Healing Touch;[@mouseover,form:4,harm,nodead][nospec:1,talent:3/1,form:4][spec:1]Lunar Strike;[spec:4]Healing Touch;[nocombat,noform:1/2]Nature's Beacon;\n/targetenemy [noexists]\n/cleartarget [dead]")
		elseif class == "DEMONHUNTER" then
		...
		if class == "SHAMAN" then
			EditMacro("WSxSGen+4",nil,nil,"#showtooltip\n/use [noexists,nocombat]Sen'jin Spirit Drum;[spec:2,talent:6/3]Sundering;[spec:2,talent:6/2]!Fury of Air;[spec:1,talent:5/3]Elemental Blast;[spec:3,talent:6/2]Cloudburst Totem;\n/targetenemy [noexists]")
		-- if class == "SHAMAN" then
		...
		elseif class == "HUNTER" then
			EditMacro("WSxSGen+4",nil,nil,"#showtooltip\n/use [spec:2,talent:4/1]Explosive Shot;[spec:2,talent:4/2]Sentinel;[spec:3,talent:6/2]Dragonsfire Grenade;[nocombat,noexists]Adopted Puppy Crate;[spec:3,@player]Tar Trap;Misdirection;\n/use [noexists,nocombat]Leather Pet Bed\n/stopspelltarget")
		elseif class == "ROGUE" then
		...
		elseif class == "DRUID" then
			EditMacro("WSxSGen+4",nil,nil,"/use [spec:1,noform:4]Moonkin Form;[spec:1,talent:5/3]Stellar Flare;[spec:4,talent:1/2]Cenarion Ward;[spec:2,noform:2]Cat Form;[spec:2]Tiger's Fury;[spec:3,noform:1]Bear Form;[spec:3,talent:1/2]Bristling Fur;Charm Woodland Creature")
		elseif class == "DEMONHUNTER" then
		...
		elseif class == "MAGE" then
			EditMacro("WSxCGen+4",nil,nil,"#showtooltip\n/use [nocombat,noexists]Faded Wizard Hat;[talent:3/1]Mirror Image;[talent:3/2]Rune of Power;\n/use [nocombat,noexists] Gastropod Shell;")
		...
		elseif class == "DEATHKNIGHT" then
			EditMacro("WSxCGen+4",nil,nil,"#showtooltip\n/use [spec:1,talent:7/1]Bonestorm;[spec:1,talent:7/2]Blood Mirror;[spec:1]Marrowrend;[spec:2]Empower Rune Weapon;[spec:3]Summon Gargoyle;\n/startattack\n/use [nocombat,noexists] Gastropod Shell")
		elseif class == "WARRIOR" then
			EditMacro("WSxCGen+4",nil,nil,"#showtooltip\n/use Battle Cry\n/use Tournament Favor\n/use [nocombat,noexists] Gastropod Shell\n/cancelaura [combat] Tournament Favor\n/startattack\n/cleartarget [dead]")
		elseif class == "DRUID" then
			EditMacro("WSxCGen+4",nil,nil,"#showtooltip\n/use [spec:1,talent:6/3]Blessing of the Ancients;[spec:1,talent:6/2]Astral Communion;[spec:2,talent:5/3]Savage Roar;[spec:4,@cursor]Efflorescence;[spec:3]Incapacitating Roar;[nocombat,noexists]Treant Form;")
		elseif class == "DEMONHUNTER" then
			EditMacro("WSxCGen+4",nil,nil,"#showtooltip\n/use [spec:1,talent:7/1]Chaos Blades;[spec:1,talent:7/2]Fel Barrage;\n/targetenemy [noexists]\n/use [nocombat,noexists] Gastropod Shell\n/startattack")
		end
		...
		elseif class == "MAGE" then
			EditMacro("WSxGen5",nil,nil,"#showtooltip\n/targetenemy [noharm]\n/use [spec:1]Arcane Barrage;[spec:2,@mouseover,harm,nodead][spec:2]Fire Blast;[spec:3,@mouseover,harm,nodead][spec:3]Ice Lance;\n/cleartarget [dead]")
		elseif class == "WARLOCK" then
			EditMacro("WSxGen5",nil,nil,"/use [mod:ctrl]Summon Infernal;[nocombat,noexists]Fire-Eater's Vial;[spec:1,talent:4/1]Phantom Singularity;[spec:1,talent:7/2]Siphon Life;[spec:1]Corruption;[spec:3]Conflagrate;[spec:2,talent:1/2]Shadowflame;Shadow bolt;\n/targetenemy [noexists]")
		elseif class == "MONK" then
		...
		elseif class == "PALADIN" then
			EditMacro("WSxGen5",nil,nil,"#showtooltip\n/use [spec:2,mod:ctrl]Ardent Defender;[spec:1,mod:ctrl]Aura Mastery;[spec:3]Templar's Verdict;[spec:2]Shield of the Righteous;[spec:1,@mouseover,help,nodead][spec:1]Holy Light;\n/use [mod:ctrl] 19\n/targetenemy [noexists]\n/cleartarget [dead]")
		elseif class == "HUNTER" then
		...
		elseif class == "DEATHKNIGHT" then
			EditMacro("WSxGen5",nil,nil,"#showtooltip\n/use [mod:ctrl,spec:3]Army of the Dead;[nocombat,noexists]Permanent Frost Essence;[spec:1]Death Strike;[spec:2]Frost Strike;[@mouseover,harm,nodead][]Death Coil;\n/startattack\n/cleartarget [dead]\n/targetenemy [noexists]")
		elseif class == "WARRIOR" then
			EditMacro("WSxGen5",nil,nil,"#showtooltip\n/use [spec:3]Last Stand;[mod]Commanding Shout;[spec:1,talent:1/2]Overpower;[spec:1,talent:6/3]Focused Rage;[spec:2]Furious Slash;[spec:3]Thunder Clap;\n/use [mod:ctrl]Gamon's Braid\n/startattack\n/cleartarget [dead]\n/targetenemy [noexists]")
		elseif class == "DRUID" then
			EditMacro("WSxGen5",nil,nil,"#showtooltip\n/use [spec:4,mod:ctrl]Incarnation: Tree of Life;[nospec:4,talent:3/3,help,nodead]Healing Touch;[form:2]Ferocious Bite;[spec:3,form:1]Maul;Solar Wrath;\n/targetenemy [noexists]\n/cleartarget [dead]")
		elseif class == "DEMONHUNTER" then
			EditMacro("WSxGen5",nil,nil,"#showtooltip\n/targetlasttarget [noexists,nocombat]\n/use [spec:2,mod:ctrl]Metamorphosis;[mod:ctrl]Darkness;[harm,dead,nocombat,nomod]Soul Inhaler;[spec:1]Chaos Strike;[spec:2]Soul Cleave;\n/startattack")
		end
		
		-- Shift+5
		if class == "SHAMAN" then
			EditMacro("WSxSGen+5",nil,nil,"#showtooltip\n/castsequence [spec:1,talent:1/3] reset=combat/5 Totem Mastery,Lava Surge\n/use [spec:3]Healing Stream Totem\n/stopmacro\n/use Bloodlust") 
		elseif class == "MAGE" then
		...
		elseif class == "PRIEST" then
			EditMacro("WSxSGen+5",nil,nil,"#showtooltip\n/focus target [@focus,noexists,nomod]\n/targetenemy\n/cleartarget [dead]\n/clearfocus [dead]\n/use [nocombat,noexists] Gastropod Shell;[help,nocombat]B. F. F. Necklace;[spec:2,talent:4/3]Symbol of Hope;[spec:2]Mass Dispel;[spec:1]Penance;Voidform;")
		elseif class == "DEATHKNIGHT" then
		...
		elseif class == "DEMONHUNTER" then
			EditMacro("WSxGen6",nil,nil,"/use [mod:ctrl] Metamorphosis; Blade Dance;\n/targetenemy [noexists]")
		end
		
		-- Shift+6
		if class == "SHAMAN" then
			EditMacro("WSxSGen+6",nil,nil,"#showtooltip\n/use [nocombat,noexists] Lava Fountain;[spec:2,talent:6/2]Fury of Air;[spec:3]Healing Tide Totem;[spec:1,talent:6/1,@cursor]Liquid Magma Totem;[spec:1,talent:1/3,@cursor][spec:1,@player]Earthquake;")
		elseif class == "MAGE" then
		...
		elseif class == "MONK" then
			EditMacro("WSxSGen+6",nil,nil,"#showtooltip\n/use [@mouseover,spec:3,harm,nodead][spec:3]Fists of Fury;[spec:2]Vivify;\n/targetenemy [spec:3]")
		elseif class == "PALADIN" then
		...
		elseif class == "HUNTER" then
			EditMacro("WSxSGen+6",nil,nil,"#showtooltip\n/use [spec:2]Bursting Shot;[spec:3,@cursor]Explosive Trap;")
		elseif class == "ROGUE" then
		...
		elseif class == "PRIEST" then
			EditMacro("WSxSGen+6",nil,nil,"#showtooltip\n/use [spec:2]Prayer of Healing;[spec:1]Power Word: Radiance;[exists,nodead]Shadowfiend;Xan'tish's Flute;\n/use Battle Standard of Coordination\n/use Cursed Feather of Ikzan")
		elseif class == "DEATHKNIGHT" then
		...
		if class == "SHAMAN" then
			EditMacro("WSxGen7",nil,nil,"#showtooltip\n/use [spec:2]Frostbrand;[spec:1,@cursor]Earthquake;Chain Lightning;\n/startattack\n/use [mod:shift]Blood fury\n/use [mod:shift] Berserking\n/use Bom'bay's Color-Seein' Sauce\n/use [noexists,nocombat]Moonfang's Paw")
		elseif class == "MAGE" then
			EditMacro("WSxGen7",nil,nil,"#showtooltip\n/use [nocombat,noexists]Pilfered Sweeper;[spec:2,talent:2/2]Blast Wave;[spec:1,talent:4/1]Supernova;[spec:1,talent:4/2]Charged up;[spec:3,talent:4/1]Ice Nova;[spec:3,talent:4/2]Frozen Touch;[spec:2]Flamestrike;")
		elseif class == "WARLOCK" then
		...
		elseif class == "HUNTER" then
			EditMacro("WSxGen7",nil,nil,"#showtooltip\n/use [nocombat,noexists]Fireworks;[spec:1]Aspect of the Wild;[spec:3,talent:1/2]Throwing Axes;[spec:2]Bursting Shot;\n/use Will of Northrend\n/use Bom'bay's Color-Seein' Sauce;")
		elseif class == "ROGUE" then
			EditMacro("WSxGen7",nil,nil,"#showtooltip\n/use [mod:shift] Barrel of Bandanas\n/use [mod:shift]Barrel of Eyepatches\n/use [nostealth,noexists,nomod] Worn Troll Dice;[spec:2,nomod]Roll the Bones;[spec:3,nomod]Shadow Blades;")
		elseif class == "PRIEST" then
			EditMacro("WSxGen7",nil,nil,"#showtooltip\n/use [mod:shift]Crate of Bobbers: Squeaky Duck;[spec:2,@cursor]Holy Word: Sanctify;[spec:3,talent:6/1][spec:1,talent:7/1]Power Infusion;[spec:1,talent:7/3]Evangelism;[noexists,nocombat]Romantic Picnic Basket;Smite;")
		elseif class == "DEATHKNIGHT" then
			EditMacro("WSxGen7",nil,nil,"#showtooltip\n/use [spec:3,pet]Dark Transformation;[spec:2]Pillar of Frost;[spec:3,nopet]Raise Dead;Blood Boil;\n/stopspelltarget")
		elseif class == "WARRIOR" then
		...
		elseif class == "DRUID" then
			EditMacro("WSxGen7",nil,nil,"#showtooltip\n/use [spec:1,talent:7/1]Fury of Elune;[spec:4,talent:7/3]Flourish;[nocombat,noexists]Spirit of Bashiok;Barkskin;")
		elseif class == "DEMONHUNTER" then
			EditMacro("WSxGen7",nil,nil,"#showtooltip\n/use Throw Glaive\n/targetenemy [noexists]")
		end
		...
		elseif class == "MAGE" then
			EditMacro("WSxQQ",nil,nil,"#showtooltip\n/stopcasting [nomod]\n/use [mod:alt,@focus,harm,nodead]Polymorph;[mod:shift]Ice Block;[@mouseover,harm,nodead][]Counterspell")
		elseif class == "WARLOCK" then
		...
		elseif class == "MONK" then
			EditMacro("WSxStuns",nil,nil,"#showtooltip\n/use [nomod,spec:3]Flying Serpent Kick;[nomod,spec:2]Transcendence;[spec:1,talent:4/2]Summon Black Ox Statue;[spec:2,talent:4/2]Song of Chi-Ji;[talent:4/3]Leg Sweep;Ring of Peace")
		elseif class == "PALADIN" then
			EditMacro("WSxStuns",nil,nil,"#showtooltip Hammer of Justice\n/use [mod:alt]Blinding Light;[nospec:1]Hammer of Justice;[@mouseover,help,nodead][]Holy Light;")
		elseif class == "HUNTER" then
			EditMacro("WSxStuns",nil,nil,"#showtooltip\n/use [mod:alt,@cursor]Flare;[spec:3,exists,nodead]Harpoon;[@cursor]!Stopping Power;\n/targetenemy [noexists]\n/startattack [combat]\n/cleartarget [dead]")
		elseif class == "ROGUE" then
			EditMacro("WSxStuns",nil,nil,"/use [stance:0,combat,spec:3]Shadow Dance;[stance:0,combat,nospec:3]Vanish;[stance:0,nocombat]Stealth;[mod:alt,@focus,harm,nodead,stance:1/2/3][stance:1/2/3][]Cheap Shot;\n/use [nostealth,spec:2,nocombat]Iron Buccaneer's Hat") 
		end
		-- Stuns Prist "E" "Stuns"
		if class == "PRIEST" then
			if race == "Troll" then
			EditMacro("WSxStuns",nil,nil,"#show\n/use [mod:alt]Mass Dispel;[spec:3]Psychic Scream;[nospec:3]Leap of Faith;\n/use [nomod] Orb of the Sin'Dorei\n/use [nomod,nocombat,noexists] Leather Love Seat")
			elseif race == "BloodElf" then
			EditMacro("WSxStuns",nil,nil,"#show\n/use [mod:alt]Mass Dispel;[spec:3]Psychic Scream;[nospec:3]Leap of Faith;\n/use [nomod] Orb of Deception\n/use [nomod,nocombat,noexists] Leather Love Seat")
			end
		end
		...
		elseif class == "MAGE" then
			EditMacro("WSxRTS",nil,nil,"#showtooltip\n/use [mod:alt]MOLL-E;[spec:3,mod:shift,nopet]Summon Water Elemental;[spec:3,mod:shift]Freeze;[spec:2,mod:shift,@player]Flamestrike;[spec:3]Cone of Cold;[spec:2]Dragon's Breath;Polymorph;\n/stopmacro [@pet,nodead]\n/script PetDismiss();")
		elseif class == "WARLOCK" then
		...
		elseif class == "HUNTER" then
			EditMacro("WSxRTS",nil,nil,"/use [mod:ctrl,nopet]Call Pet 3;[mod:shift,@cursor]Tar Trap;[spec:3,nomod]Wing Clip;[mod:alt,@focus,harm,nodead][nomod]Concussive Shot;[mod:alt]MOLL-E;\n/targetenemy [noharm]")
		elseif class == "ROGUE" then
			EditMacro("WSxRTS",nil,nil,"/use [mod:alt,@focus,harm,nodead]Gouge;[mod:alt]MOLL-E;[spec:1,mod:shift,talent:6/1]Agonizing Poison;[@mouseover,exists,nodead]Shadowstep;[spec:2]Pistol Shot;[spec:3]Shuriken Toss;[spec:1]Poisoned Knife;\n/targetenemy [noharm]")
		elseif class == "PRIEST" then
			EditMacro("WSxRTS",nil,nil,"/use [mod:alt]MOLL-E;[mod:ctrl,nospec:3,talent:2/1,@player][nomod,nospec:3,talent:2/1,@cursor]Angelic Feather;[mod:shift,spec:1]Psychic Scream;[spec:2,talent:2/2]Body and Mind;[@mouseover,help,nodead][]Power Word: Shield\n/stopspelltarget")
		elseif class == "DEATHKNIGHT" then
		...
		if class == "HUNTER" then
			if petspec == 1 then
				EditMacro("WSxClassT",nil,nil,"/targetenemy [noexists]\n/use [help]Swapblaster;[spec:3]Hatchet Toss;\n/petattack [@mouseover,harm][]\n/use [nocombat,noexists]Trans-Dimensional Bird Whistle;\n/use Dash")
			elseif petspec == 2 then
		...
		elseif class == "WARLOCK" then
			EditMacro("WSxGenF",nil,nil,"#showtooltip\n/focus [@mouseover,exists] mouseover\n/stopmacro [@mouseover,exists]\n/stopcasting [nomod,nopet]\n/use [mod:alt] Eye of Kilrogg;[@focus,pet:felguard/wrathguard,harm,nodead]Axe Toss;[@focus,harm,exists,nodead]Command Demon;Fishing;")
		elseif class == "MONK" then
			EditMacro("WSxGenF",nil,nil,"#showtooltip\n/focus [@mouseover,exists] mouseover\n/stopmacro [@mouseover,exists]\n/use [mod:alt]Farwater Conch;[spec:2,@focus,harm,nodead]Paralysis;[@focus,harm,nodead]Spear Hand Strike;Fishing;\n/targetenemy [noexists]")
		elseif class == "PALADIN" then
			EditMacro("WSxGenF",nil,nil,"#showtooltip Blessing of Freedom\n/focus [@mouseover,exists] mouseover\n/stopmacro [@mouseover,exists]\n/use [spec:2,mod:alt,@focus,harm,nodead]Avenger's Shield;[mod:alt]Farwater Conch;[@focus,harm,nodead]Rebuke;Fishing;")
		elseif class == "HUNTER" then
			EditMacro("WSxGenF",nil,nil,"#showtooltip Tar Trap\n/focus [@mouseover,exists] mouseover\n/stopmacro [@mouseover,exists]\n/use [mod:alt]!Eagle Eye;[spec:3,@focus,harm,nodead]Muzzle;[@focus,harm,nodead]Counter Shot;Fishing;")
		elseif class == "ROGUE" then
		...
		elseif class == "PRIEST" then
			EditMacro("WSxGenF",nil,nil,"#show\n/focus [@mouseover,exists] mouseover\n/stopmacro [@mouseover,exists]\n/use [mod:alt,@focus,exists]Shackle Undead;[mod:alt,exists,nodead]Mind Vision;[mod:alt]Farwater Conch;[spec:3,@focus,harm,nodead]Silence;[help,nodead]True Love Prism;Fishing;")
		elseif class == "DEATHKNIGHT" then
			EditMacro("WSxGenF",nil,nil,"#showtooltip\n/focus [@mouseover,exists] mouseover\n/stopmacro [@mouseover,exists]\n/use [mod:alt,@focus,harm,nodead]Leap;[@focus,harm,nodead]Mind Freeze;Fishing;\n/petattack [mod:alt,@focus,harm,nodead]")
		elseif class == "WARRIOR" then
			EditMacro("WSxGenF",nil,nil,"#showtooltip Berserker Rage\n/focus [@mouseover,exists] mouseover\n/stopmacro [@mouseover,exists]\n/use [@focus,harm,nodead]Pummel;Survey;")
		elseif class == "DRUID" then
		...
		elseif class == "DEMONHUNTER" then
			EditMacro("WSxGenF",nil,nil,"#showtooltip Spectral Sight\n/focus [@mouseover,exists] mouseover\n/stopmacro [@mouseover,exists]\n/use [mod:alt]Spectral Sight;[@focus,harm,nodead]Consume Magic;")
		end
		...
		elseif class == "MAGE" then
			EditMacro("WSxSGen+F",nil,nil,"#showtooltip Familiar Stone\n/use Arcane Familiar Stone\n/use Fiery Familiar Stone\n/use Icy Familiar Stone\n/use Familiar Stone")
		elseif class == "WARLOCK" then
		...
		elseif class == "MONK" then
			EditMacro("WSxSGen+F",nil,nil,"#showtooltip\n/cleartarget [spec:1]\n/target [nospec:2,talent:4/2]Black Ox;\n/use [nospec:2,talent:4/2,help]Provoke;[nospec:2,talent:4/2,nohelp,@curso]rSummon Black Ox Statue;")
		elseif class == "PALADIN" then
			EditMacro("WSxSGen+F",nil,nil,"/use [spec:2,@focus,harm,nodead]Avenger's Shield")
		elseif class == "HUNTER" then
		...
		elseif class == "ROGUE" then
			EditMacro("WSxSGen+F",nil,nil,"#showtooltip\n/use [nocombat,noexists]Slightly-Chewed Insult Book;[spec:2,@focus,harm,nodead]Between the Eyes;[nospec:2,@focus,harm,nodead,mod:alt][nospec:2]Shadowstep\n/use [nospec:2,@focus,harm,nodead,mod:alt][nospec:2]Kick")
		elseif class == "PRIEST" then
		...
		elseif class == "DEMONHUNTER" then
			EditMacro("WSxSGen+F",nil,nil,"#showtooltip\n/use [spec:2,pvptalent:6/2]Demonic Trample;[spec:2,pvptalent:6/3]Illidan's Grasp;[pvptalent:6/2]Rain from Above;[pvptalent:6/3]Mana Break;")
		end
		...
		elseif class == "DRUID" then
			EditMacro("WSxCGen+F",nil,nil,"#showtooltip [spec:2/3]Stampeding Roar;Mushroom Chair\n/use [spec:3,pvptalent:4/3]Demoralizing Roar; Mushroom Chair;")
		elseif class == "DEMONHUNTER" then
		...
		elseif class == "DRUID" then
			EditMacro("WSxCAGen+F",nil,nil,"#showtooltip [nocombat,noexists] Tear of the Green Aspect; Barkskin \n/use Tear of the Green Aspect")
		elseif class == "DEMONHUNTER" then
		...
		if class == "SHAMAN" then
			EditMacro("WSxGG",nil,nil,"/use [@mouseover,harm,nodead]Purge;[spec:3,@mouseover,help,nodead][spec:3]Purify Spirit;[@mouseover,help,nodead][]Cleanse Spirit;\n/targetenemy [noexists]\n/use Poison Extraction Totem")
		elseif class == "MAGE" then
			EditMacro("WSxGG",nil,nil,"#showtooltip\n/targetenemy [noharm]\n/use [mod:alt]Darkmoon Gazer;[@mouseover,harm,nodead][]Spellsteal;")
		elseif class == "WARLOCK" then
			EditMacro("WSxGG",nil,nil,"#showtooltip\n/use [mod:alt,pet:Voidwalker]Shadow Bulwark;[pet:Succubus]Seduction;[pet:Shivarra]Mesmerize;[pet:felguard/wrathguard]Axe Toss;[@mouseover,help,nodead,pet:imp][help,nodead,pet:imp][@player,pet:imp]Singe Magic;Command Demon")
		elseif class == "MONK" then
			EditMacro("WSxGG",nil,nil,"#showtooltip\n/use [mod:alt]Nimble Brew;[@mouseover,help,nodead][]Detox;")
		elseif class == "PALADIN" then
			EditMacro("WSxGG",nil,nil,"#showtooltip\n/use [mod:alt]Divine Shield;[spec:1,@mouseover,help,nodead][spec:1]Cleanse;[@mouseover,help,nodead][]Cleanse Toxins;\n/cancelaura [mod:alt]Divine Shield\n/cancelaura [mod:alt]Hand of Protection")
		elseif class == "HUNTER" then
			EditMacro("WSxGG",nil,nil,"/use [mod:alt]Bullheaded;[harm,nocombat]Hozen Idol;[nocombat]Critter Hand Cannon;[pvptalent:4/1]Viper Sting;[pvptalent:4/2]Scorpid Sting;[pvptalent:4/3]Spider Sting;\n/target [nocombat,noexists]Squirrel")
		elseif class == "ROGUE" then 
			EditMacro("WSxGG",nil,nil,"#showtooltip\n/use [spec:1,pvptalent:5/3]Shiv;[spec:3,pvptalent:5/3]Cold Blood;[spec:2,harm,nodead]Gouge;[noexists]Pick Lock;")
		elseif class == "PRIEST" then
			EditMacro("WSxGG",nil,nil,"#showtooltip\n/use [@mouseover,harm,nodead]Dispel Magic;[nospec:3,@mouseover,help,nodead][nospec:3]Purify;[@mouseover,help,nodead][]Purify Disease;")
		elseif class == "DEATHKNIGHT" then
			EditMacro("WSxGG",nil,nil,"/use [mod:alt,@focus,harm,nodead]Dark Simulacrum;[spec:3,nopet]Raise Dead;[nocombat,noexists]Lilian's Warning Sign[spec:3,talent:4/1,pet:Abomination]Hook;[spec:3,pet:Ghoul]Leap;[@focus,harm,nodead]Death Grip;")
		elseif class == "WARRIOR" then
			EditMacro("WSxGG",nil,nil,"#showtooltip [talent:3/3]Avatar;[talent:3/2]Rend;[spec:3]Demoralizing Shout;Whirlwind;\n/targetenemy [noharm]")
		elseif class == "DRUID" then
			EditMacro("WSxGG",nil,nil,"#showtooltip\n/use [mod:alt]Stampeding Roar;[spec:4,@mouseover,help,nodead][spec:4]Nature's Cure;[@mouseover,help,nodead][]Remove Corruption;")
		elseif class == "DEMONHUNTER" then
			EditMacro("WSxGG",nil,nil,"#showtooltip\n/use [pvptalent:4/2]Reverse Magic;[pvptalent:4/3]Eye of Leotheras;")
		end
		...
		elseif class == "MONK" then
			EditMacro("WSxSGen+G",nil,nil,"#showtooltip\n/use [noexists,nocombat] Flaming Hoop;Grappling Hook;")
		elseif class == "PALADIN" then
			EditMacro("WSxSGen+G",nil,nil,"#showtooltip\n/use [noexists,nocombat] Flaming Hoop;Hammer of Justice;\n/targetenemy [noexists]")
		elseif class == "HUNTER" then
			EditMacro("WSxSGen+G",nil,nil,"/use [nopet]Call Pet 4;[pet:Spirit Beast]Spirit Walk;[pet:Crane,@mouseover,exists][pet:Crane,exists]Gift of Chi-Ji;[pet:Crane]Trick;[pet:Fox]Play;[pet:Devilsaur]Feast;[pet:Rylak]Updraft;[pet:Crab]Harden Shell;\n/use [noexists,nocombat] Flaming Hoop")
		elseif class == "ROGUE" then
			EditMacro("WSxSGen+G",nil,nil,"#showtooltip\n/use [noexists,nocombat] Flaming Hoop;[spec:2]Between the Eyes;Kidney Shot;\n/targetenemy [noexists]")
		elseif class == "PRIEST" then
		...
		if class == "SHAMAN" then
			EditMacro("WSxDef",nil,nil,"/use [combat,mod:ctrl]Treessassin's Guise;[mod:ctrl]Repurposed Fel Focuser;[mod:shift]Grand Expedition Yak;Astral Shift\n/use Seafarer's Slidewhistle\n/use [mod:shift,@cursor]Spirit Link Totem;[spec:3]Waterspeaker's Totem;Lightning Shield")
		elseif class == "MAGE" then
			EditMacro("WSxDef",nil,nil,"/use [mod]Seafarer's Slidewhistle\n/use Dalaran Initiates' Pin\n/use [mod:ctrl,nocombat]Repurposed Fel Focuser;[mod:shift]Grand Expedition Yak;[spec:1,mod:ctrl][spec:1,nocombat]Greater Invisibility;[mod:ctrl][nocombat]Invisibility;!Ice Block;")
		elseif class == "WARLOCK" then
			EditMacro("WSxDef",nil,nil,"#show\n/use [mod:alt,@cursor]Demonic Gateway;[mod:ctrl,combat]Treessassin's Guise;[mod:ctrl]Repurposed Fel Focuser;[mod:shift]Grand Expedition Yak;Unending Resolve\n/use [mod:ctrl]Haunting Memento\n/use [mod:ctrl]Seafarer's Slidewhistle\n/stopspelltarget")
		elseif class == "MONK" then
			EditMacro("WSxDef",nil,nil,"#showtooltip\n/use [mod:ctrl]Repurposed Fel Focuser;[combat,mod:shift]Fortifying Brew;[talent:5/1]Healing Elixir;[nospec:1,talent:5/2]Diffuse Magic;[talent:5/3]Dampen Harm;\n/use [mod:ctrl]Seafarer's Slidewhistle;[mod:shift]Grand Expedition Yak")
		elseif class == "PALADIN" then
			EditMacro("WSxDef",nil,nil,"/use [mod:ctrl]Seafarer's Slidewhistle;[mod:shift]Grand Expedition Yak;[spec:3]Divine Shield;[spec:1]Divine Protection;Guardian of Ancient Kings\n/use [mod:ctrl]Greater Blessing of Kings;[mod:shift]Blessing of Protection\n/use [mod:ctrl]Repurposed Fel Focuser")
		elseif class == "HUNTER" then
			EditMacro("WSxDef",nil,nil,"#showtooltip\n/use [mod:alt]Hunter's Call;[mod:ctrl]Repurposed Fel Focuser;[mod]Play Dead;[nomod]Feign Death\n/use [mod:shift]Grand Expedition Yak;[mod]Seafarer's Slidewhistle\n/use [nocombat,mod:ctrl]!Camouflage")
		elseif class == "ROGUE" then
			EditMacro("WSxDef",nil,nil,"/use [mod:alt,spec:1] Deadly Poison\n/use [mod:ctrl,combat]Vanish;[mod:ctrl,stance:0]Repurposed Fel Focuser;[mod:shift]Grand Expedition Yak;[stance:0,nocombat]Stealth;[spec:2,combat]Riposte;[spec:1/3,combat]Evasion;[stance:1]Shroud of Concealment;")
		elseif class == "PRIEST" then
			EditMacro("WSxDef",nil,nil,"/use [mod:ctrl,combat]Treessassin's Guise;[mod:ctrl]Repurposed Fel Focuser;[mod:shift]Grand Expedition Yak;[spec:3]Dispersion;[spec:2]Guardian Spirit;[spec:1]Pain Suppression;\n/use [mod:ctrl]Power Word: Fortitude\n/use Seafarer's Slidewhistle")
		elseif class == "DEATHKNIGHT" then
			EditMacro("WSxDef",nil,nil,"/use [mod:ctrl,combat]Treessassin's Guise;[mod:ctrl]Repurposed Fel Focuser;[mod:shift]Grand Expedition Yak;Icebound Fortitude\n/use Seafarer's Slidewhistle\n/use [spec:1,mod:shift]Dancing Rune Weapon;[mod:shift]Corpse Shield")
		elseif class == "WARRIOR" then 						 
			EditMacro("WSxDef",nil,nil,"/use [mod:ctrl,combat]Treessassin's Guise;[mod:ctrl]Repurposed Fel Focuser;[mod:shift]Grand Expedition Yak;[spec:1]Die By The Sword;[spec:2]Enraged Regeneration;Shield Wall\n/use [mod:ctrl]Defensive Stance;\n/use Seafarer's Slidewhistle;")
		elseif class == "DRUID" then
			EditMacro("WSxDef",nil,nil,"#showtooltip\n/use [mod:ctrl,nostealth]Repurposed Fel Focuser;[mod:shift,nocombat]Grand Expedition Yak;[spec:4]Ironbark;[spec:2/3]Survival Instincts;\n/use [mod:ctrl]!Prowl;[mod:shift][nomod,spec:1]Barkskin\n/use [nostealth]Seafarer's Slidewhistle")
		elseif class == "DEMONHUNTER" then
			EditMacro("WSxDef",nil,nil,"#showtooltip\n/use [mod:ctrl,combat]Treessassin's Guise;[mod:ctrl]Repurposed Fel Focuser;[mod:shift]Grand Expedition Yak;[spec:2]Fiery Brand;Blur;\n/use Seafarer's Slidewhistle\n/use Shard of Archstone")
		end
		
		-- Macro "GND" for button X, Absorbing, Reflect or External Defensive Button.
		if class == "SHAMAN" then
			EditMacro("WSxGND",nil,nil,"/use [mod:alt]Xan'tish's Flute;[mod:ctrl]Astral Recall;[spec:2,mod:shift]Spirit Walk;[spec:1,pvptalent:5/3]Control of Lava;[spec:3,pvptalent:5/3]Grounding Totem;[spec:3]Healing Stream Totem;\n/use Void Totem")
		elseif class == "MAGE" then
		...
		elseif class == "MONK" then
			EditMacro("WSxGND",nil,nil,"#showtooltip\n/use [mod:ctrl]Zen Pilgrimage;[mod:shift]Transcendence: Transfer;[spec:1,pvptalent:5/1]Guard;[spec:1,pvptalent:5/3]Craft: Nimble Brew;[spec:1]Fortifying Brew;[spec:3]Touch of Karma;[spec:2,@mouseover,help,nodead][spec:2,nodead]Life Cocoon;")
		elseif class == "PALADIN" then
		...
		elseif class == "ROGUE" then
			EditMacro("WSxGND",nil,nil,"#showtooltip [mod:alt,spec:1,pvptalent:5/3]Create: Crimson Vial;[mod:ctrl]Bribe;Sprint;\n/use [spec:1,mod:alt]Crippling Poison;[spec:2,mod:ctrl]Bribe;[mod:shift]Sprint;Feint;\n/use [nostealth,mod:shift]Thistleleaf Branch")
		elseif class == "PRIEST" then
			EditMacro("WSxGND",nil,nil,"/use [mod:ctrl]Mind Control;[mod:shift]Fade;[spec:2,@mouseover,help,nodead][spec:2]Prayer of Mending;[@mouseover,help,nodead][]Power Word: Shield;\n/use [nocombat]Bubble Wand\n/use [nocombat]Void Totem\n/cancelaura Bubble Wand")
		elseif class == "DEATHKNIGHT" then
		...
		elseif class == "PALADIN" then
			EditMacro("WSxCC",nil,nil,"/use [mod:ctrl,talent:3/2]Repentance;[spec:3,talent:5/2]Eye for an Eye;[spec:3,talent:5/3]Word of Glory;[spec:1,talent:1/1]Bestow Faith;[spec:1,talent:1/2]Light's Hammer;[@mouseover,spec:2,talent:5/1,help,nodead][spec:2]Light of the Protector;Judgment;")
		elseif class == "HUNTER" then
		...
		elseif class == "PRIEST" then
			EditMacro("WSxCC",nil,nil,"#showtooltip\n/use [mod:ctrl]Shackle Undead;[spec:1,@mouseover,help,nodead][spec:1]Plea;[spec:2,mod:shift,talent:4/3]Symbol of Hope;[spec:2,@mouseover,help,nodead][spec:2]Renew;[mod:shift][]Shadowfiend;\n/cancelaura X-Ray Specs")
		elseif class == "DEATHKNIGHT" then
		...
		if class == "SHAMAN" then
			EditMacro("WSxMove",nil,nil,"/use [talent:2/3]Wind Rush Totem;[nospec:2,talent:2/1]Gust of Wind;[spec:1,talent:2/2]Ancestral Guidance;[spec:2,talent:2/1]Rainfall;[spec:2,talent:2/2]Feral Lunge;Ghost Wolf\n/use Deceptia's Smoldering Boots\n/targetenemy [noexists]\n/cleartarget [dead]")
		elseif class == "MAGE" then
			EditMacro("WSxMove",nil,nil,"/use Blink\n/dismount [mounted]")
		elseif class == "WARLOCK" then
			EditMacro("WSxMove",nil,nil,"#showtooltip\n/use [spec:2,talent:6/2]Grimoire: Felhunter;[spec:3,@mouseover,harm,nodead][spec:3]Havoc;[@mouseover,harm,nodead][]Soul Swap") 
		elseif class == "MONK" then
			EditMacro("WSxMove",nil,nil,"#showtooltip\n/use Roll")
		elseif class == "PALADIN" then
		...
		elseif class == "ROGUE" then
			EditMacro("WSxMove",nil,nil,"/use [spec:1/3]Shadowstep;[spec:2,talent:2/1]Grappling Hook;Sprint;\n/targetenemy [noexists]")
		elseif class == "PRIEST" then
		...
		elseif class == "WARRIOR" then
			EditMacro("WSxMove",nil,nil,"/use !Heroic Leap")
		elseif class == "DRUID" then
		...
		elseif class == "DEMONHUNTER" then
			EditMacro("WSxMove",nil,nil,"#showtooltip\n/use [spec:2]Infernal Strike;Vengeful Retreat;")
		end
		...
		if class == "WARLOCK" then
			EditMacro("WSxSGen+V",nil,nil,"/castrandom [flyable]Headless Horseman's Mount,Netherlord's Accursed Wrathsteed,Netherlord's Chaotic Wrathsteed,Netherlord's Brimstone Wrathsteed\n/castrandom [noflyable]Vicious Skeletal Warhorse,Wild Dreamrunner,Lucid Nightmare,Illidari Felstalker") 
		elseif class == "MONK" then
		...
		if class == "PALADIN" then
			if playerspec == 1 then
				EditMacro("WSxSGen+V",nil,nil,"/castrandom [flyable] Highlord's Valorous Charger, Highlord's Golden Charger, Invincible\n/castrandom Swift Zulian Tiger, Argent Warhorse, Summon Thalassian Warhorse, Highlord's Valorous Charger, Invincible, Wild Dreamrunner")
			elseif playerspec == 2 then
				EditMacro("WSxSGen+V",nil,nil,"/castrandom [flyable] Highlord's Vigilant Charger, Highlord's Golden Charger, Invincible\n/castrandom Swift Zulian Tiger, Argent Warhorse, Summon Thalassian Warhorse, Highlord's Vigilant Charger, Invincible, Wild Dreamrunner")
			else
				EditMacro("WSxSGen+V",nil,nil,"/castrandom [flyable] Highlord's Vengeful Charger, Highlord's Golden Charger, Invincible\n/castrandom Swift Zulian Tiger, Argent Warhorse, Summon Thalassian Warhorse, Highlord's Vengeful Charger, Invincible, Wild Dreamrunner")
			end
		end
		...
		if class == "PRIEST" then
			if playerspec == 1 then
				EditMacro("WSxSGen+V",nil,nil,"/castrandom [flyable] High Priest's Lightsworn Seeker\n/use [spec:1]Trained Meadowstomper\n/use Death's Door Charm\n/dismount [mounted]")
			elseif playerspec == 2 then
		...
		if class == "SHAMAN" then
			EditMacro("WSxCGen+V",nil,nil,"/use [mod:alt]Orgrimmar Interceptor;[noexists,swimming] Barnacle-Encrusted Gem;[nocombat,noexists]Silversage Incense;[exists][]Water Walking;\n/use [nomod]Panflute of Pandaria") 
		elseif class == "MAGE" then
		...
		elseif class == "MONK" then
			EditMacro("WSxCGen+V",nil,nil,"#showtooltip [mod:alt]Orgrimmar Interceptor;[spec:1]Fortifying Brew;[spec:2]Thunder Focus Tea;[spec:3]Storm, Earth, and Fire;\n/use [swimming] Barnacle-Encrusted Gem;Zen Flight;\n/use [nostealth,nomod]Panflute of Pandaria")
		elseif class == "PALADIN" then
			EditMacro("WSxCGen+V",nil,nil,"#showtooltip\n/use [mod:alt]Orgrimmar Interceptor;[swimming] Barnacle-Encrusted Gem;Divine Steed;\n/use [nostealth,nomod]Panflute of Pandaria")
		elseif class == "HUNTER" then
			EditMacro("WSxCGen+V",nil,nil,"/use [mod:alt]Orgrimmar Interceptor;[spec:1,pet,nopet:Water Strider]Dismiss Pet;[spec:1,nopet]Call Pet 1;[spec:1,pet:Water Strider]Surface Trot;Barnacle-Encrusted Gem;\n/use Barnacle-Encrusted Gem\n/use [nostealth,nomod]Panflute of Pandaria")
		elseif class == "ROGUE" then
		...
		-- Macro "B" for button B and Shift+B, Earth Shield, Holy Shock on Tank etc. Need research on effective /cast @friendlytarget macros. Is it possible to use "/targetexact "..B" Earth Shield") - Edra Fixet syntax error with escaping quotes with \"\", genious; Have Ctrl+Alt+B keybind for WSxCAGen+B, then natural B for executing macro#2 with shift modifier allowed. Re-assign keybind for Openallbags and open character pane.
			...
			elseif class == "WARLOCK" then
				EditMacro("WSxGen+B",nil,nil,"#showtooltip\n/use [spec:1,pvptalent:5/3]Soulburn;[spec:3,pvptalent:5/3,mod:shift]Firestone;[spec:3,talent:6/2]Grimoire: Felhunter;[pvptalent:5/3]Singe Magic;[pvptalent:5/2]Call Felhunter;")
				EditMacro("WSxCAGen+B",nil,nil,"")
			elseif class == "MONK" then
			...
			elseif class == "HUNTER" then
				EditMacro("WSxCAGen+B",nil,nil,"")
				EditMacro("WSxGen+B",nil,nil,"#showtooltip\n/targetenemy [noexists]\n/use [nocombat,noexists][nospec:3]Fetch Ball;[spec:3]Harpoon;")
			elseif class == "ROGUE" then
				EditMacro("WSxCAGen+B",nil,nil,"/run if not InCombatLockdown()then local B=UnitName(\"target\") EditMacro(\"WSxGen+B\",nil,nil,\"/use [@\"..B..\"]Tricks of the Trade\", nil)print(\"Tank set to : \"..B)else print(\"Cannot change assist now!\")end")
			end
		...
		-- CTRL+ALT+B for Prist, "B" set frame for Maintank
			...
			elseif class == "WARRIOR" then
				...
				EditMacro("WSxCAGen+B",nil,nil,"")
			elseif class == "DRUID" then
				EditMacro("WSxCAGen+B",nil,nil,"/run if not InCombatLockdown()then local B=UnitName(\"target\") EditMacro(\"WSxGen+B\",nil,nil,\"/use [mod:shift,@\"..B..\"]Regrowth;[@\"..B..\"]Lifebloom\", nil)print(\"Tank set to : \"..B)else print(\"Cannot change assist now!\")end")
			elseif class == "DEMONHUNTER" then
				EditMacro("WSxGen+B",nil,nil,"#showtooltip Glide")
				EditMacro("WSxCAGen+B",nil,nil,"")
			end
		...
		-- Macro "N" for button N and Shift+N, Earth Shield, Holy Shock on Tank etc. Need research on effective /cast @friendlytarget macros. Is it possible to use "/targetexact "..N" Earth Shield") - Edra Fixet syntax error with escaping quotes with \"\", genious; Have Ctrl+Alt+N keybind for WSxCAGen+N, then natural N for executing macro#2 with shift modifier allowed. Re-assign keybind for Openallbags and open character pane.
			...
			elseif class == "HUNTER" then
				EditMacro("WSxCAGen+N",nil,nil,"/run if not InCombatLockdown()then local N=UnitName(\"target\") EditMacro(\"WSxGen+N\",nil,nil,\"/use [@\"..N..\"]Misdirection\", nil)print(\"Tank set to : \"..N)else print(\"Error: Combat Lock!\")end")
				EditMacro("WSxGen+N",nil,nil,"#showtooltip [spec:3]Harpoon;Misdirection\n/use [spec:3] Harpoon\n/targetenemy [noexists]")
			elseif class == "ROGUE" then
				EditMacro("WSxCAGen+N",nil,nil,"/run if not InCombatLockdown()then local N=UnitName(\"target\") EditMacro(\"WSxGen+N\",nil,nil,\"/use [@\"..N..\"]Tricks of the Trade\", nil)print(\"Tank#2 set to : \"..N)else print(\"Cannot change assist now!\")end")
			end
		...
		-- Prist code for "N" offtank bind.
			...
			elseif class == "WARRIOR" then
				...
				EditMacro("WSxCAGen+N",nil,nil,"")
			elseif class == "DRUID" then
				EditMacro("WSxCAGen+N",nil,nil,"/run if not InCombatLockdown()then local N=UnitName(\"target\") EditMacro(\"WSxGen+N\",nil,nil,\"/use [mod:shift,@\"..N..\"]Regrowth;[@\"..N..\"]Lifebloom\", nil)print(\"Tank#2 set to : \"..N)else print(\"Cannot change assist now!\")end")
			elseif class == "DEMONHUNTER" then
		...
		-- Addon to change content of Extra Action Button/Boss Button Depending on Zone
